...
type UpstreamRxMessage struct {
	...
	Agents	[]Agent_rx	`json:"agent"`	// 上流側エージェント計測値格納用の構造体配列
}
...
type AgentTxMessage struct {
	...
	Agents		[]Agent_tx	`json:"agent"`		// 隣接エージェント(j)の情報格納用の構造体配列
}
		...
		for{
			...
			case <- tm.tick_ch: // Tickerが発火していたら処理に入る
				...
				*/
				...
				if string(msgj) != "null" {
					fmt.Println(string(msgj)) // デバッグ用表示
				}
